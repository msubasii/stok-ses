...
from storage import stok_guncelle, stok_getir, eksikleri_getir, tedarikci_mesaji_olustur
...
async def stok_isle(input: MesajInput):
    ...
    return {"urunler": urunler, "stok": stok_getir()}

@app.get("/stok")
def stok_listesi():
    return {"stok": stok_getir()}

@app.get("/stok/eksikler")
def eksikler():
    return {"eksikler": eksikleri_getir()}
